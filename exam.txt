...
SELECT 
	...
	CAST(MIN(avg_math_4_score) as DECIMAL(12,2)) Minimum_Score,
	CAST(MAX(avg_math_4_score) as DECIMAL(12,2)) Maximum_Score
FROM
	naep
GROUP BY
	state
...
SELECT 
	CAST(AVG(avg_math_4_score) as DECIMAL(12,2))
FROM
	naep
WHERE
	year = 2000; 
...
SELECT
	naep.state, CAST(naep.avg_math_4_score AS DECIMAL(12,2)), finance.total_expenditure
FROM
	naep
LEFT JOIN finance ON naep.id = finance.id
WHERE
	naep.year = 2000 and naep.avg_math_4_score IS NOT NULL
ORDER BY 
